...
export default function DashboardTab() {
  ...
  const handleSaveEdits = async (inquiryId) => {
    ...
    try {
      ...
      if (data.success) {
        showNotification(`Inquiry ${inquiryId} updated successfully`);
        fetchInquiries();
  ...
  const productDistribution = inquiries.reduce((acc, inq) => {
    const prod = inq.product.split(' (')[0];
  ...
  return (
    <div className="admin-dashboard">
      <div className="dashboard-header" style={{ marginBottom: '1.5rem', display: 'flex', justifyContent: 'space-between' }}>
        <div className="dashboard-title">
          <h2 style={{ fontSize: '1.75rem', fontWeight: 'bold' }}>Executive Analytics Dashboard</h2>
          <p style={{ color: 'var(--text-secondary)' }}>Review metrics, track status, and export client signboard inquiries fetched directly from MongoDB Atlas.</p>
        </div>
      ...
      <div className="metrics-grid">
        <div className="metric-card">
          ...
          <div className="metric-icon metric-1"><i className="fa-solid fa-inbox"></i></div>
        </div>
        <div className="metric-card">
          <div className="metric-details">
          ...
          <div className="metric-icon metric-2"><i className="fa-solid fa-hourglass-half"></i></div>
        </div>
        <div className="metric-card">
          <div className="metric-details">
            <h4>Contacted</h4>
          ...
          <div className="metric-icon metric-3"><i className="fa-solid fa-square-phone"></i></div>
        </div>
        <div className="metric-card">
          <div className="metric-details">
            <h4>Completed</h4>
          ...
          <div className="metric-icon metric-4"><i className="fa-solid fa-circle-check"></i></div>
        </div>
      </div>
      ...
      <div className="inquiries-panel" style={{ background: 'var(--bg-primary)', padding: '1.5rem', borderRadius: 'var(--radius-md)', border: '1px solid var(--card-border)' }}>
        <div className="panel-controls" style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
          <div className="search-box" style={{ flex: 1, maxWidth: '300px' }}>
            <input
              ...
              style={{ width: '100%', padding: '0.5rem', borderRadius: '8px', border: '1px solid var(--border-glass)', background: 'var(--bg-secondary)', color: 'var(--text-primary)' }}
            />
          </div>
          
          <div className="filter-controls-group" style={{ display: 'flex', gap: '1rem' }}>
            <select
              className="dashboard-select"
              value={filterProduct}
              onChange={(e) => setFilterProduct(e.target.value)}
              style={{ padding: '0.5rem', borderRadius: '8px', border: '1px solid var(--border-glass)', background: 'var(--bg-secondary)', color: 'var(--text-primary)' }}
            >
        ...
        <div className="table-wrapper">
          {loading ? (
            ...
            <div className="table-empty-state" style={{ display: 'block', textAlign: 'center', padding: '4rem' }}>
              <div className="table-empty-state-icon"><i className="fa-solid fa-folder-open" style={{ fontSize: '3rem', color: 'var(--text-muted)' }}></i></div>
              <h4 style={{ marginTop: '1rem' }}>No Inquiries Found</h4>
            </div>
          ) : (
            <table className="inquiries-table" style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ borderBottom: '1px solid var(--border-glass)', textAlign: 'left' }}>
                  <th style={{ padding: '1rem' }}>Date</th>
                  <th style={{ padding: '1rem' }}>Client Name</th>
                  <th style={{ padding: '1rem' }}>Product Area</th>
              ...
              <tbody>
                {filteredInquiries.map(inq => {
                  ...
                  return (
                    <tr key={inq._id} style={{ borderBottom: '1px solid var(--border-glass)' }}>
                      <td style={{ padding: '1rem', fontSize: '0.85rem', color: 'var(--text-muted)' }}>{dateFormatted}</td>
                      ...
                      <td style={{ padding: '1rem' }}><span className={`badge ${priorityClass}`}>{inq.priority}</span></td>
                      <td style={{ padding: '1rem' }}><span className={`badge ${statusClass}`}>{inq.status}</span></td>
          ...
          <div className="modal-card">
            ...
            <div className="modal-body">
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', borderBottom: '1px solid var(--card-border)', paddingBottom: '1.5rem' }}>
                ...
                <div className="detail-row">
                  <span className="detail-label">Phone Call</span>
                  <span className="detail-value">{activeInquiry.phone}</span>
                </div>
              </div>

              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1.5rem', borderBottom: '1px solid var(--card-border)', paddingBottom: '1.5rem', marginTop: '1.5rem' }}>
                <div className="detail-row">
              ...
              {/* Admin Status */}
              <div style={{ background: 'rgba(255, 87, 34, 0.05)', border: '1px solid rgba(255, 87, 34, 0.15)', padding: '1.5rem', borderRadius: 'var(--radius-md)', display: 'flex', flexDirection: 'column', gap: '1.25rem', marginTop: '1.5rem' }}>
                <h4 style={{ fontSize: '0.9rem', color: 'var(--brand-color)', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  <i className="fa-solid fa-user-gear"></i> Admin Internal Controls
                </h4>
                ...
                <div className="form-group">
                  ...
                  <select
                    ...
                    style={{ background: 'var(--bg-secondary)', width: '100%', padding: '0.5rem' }}
                    value={modalStatus}
                ...
                <div className="form-group">
                  ...
                  <textarea
                    ...
                    style={{ background: 'var(--bg-secondary)', minHeight: '80px', width: '100%', padding: '0.5rem' }}
                    value={modalNotes}
                ...
                >
                  {actionLoading ? 'Saving...' : 'Save Administrative Edits'}
                </button>
              </div>
            </div>
          </div>
